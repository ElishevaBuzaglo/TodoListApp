...
namespace TodoApi.Controllers
{
    ...
    public class ItemsController : ControllerBase
    {
        ...
        // }
[HttpPost]
public async Task<ActionResult<Item>> Post([FromBody] ItemDTO newItemDto)
{
    var createdItem = await _itemService.CreateAsync(newItemDto, GetUserId());
    return CreatedAtAction(nameof(Get), new { id = createdItem.Id }, createdItem);
}
        // [HttpPut("{id}")]
        ...
        // }
[HttpPut("{id}")]
public async Task<IActionResult> Put(int id, [FromBody] ItemDTO inputDTO)
{
    if (await _itemService.UpdateAsync(id, inputDTO, GetUserId())) return NoContent();
    return NotFound();
}
        [HttpDelete("{id}")]
